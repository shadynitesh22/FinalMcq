 ...
 * @author Mili Risal
 */
public class historyExams extends JFrame implements ActionListener{
    ...
    public historyExams(){
        ...
        lblwelcome=new JLabel("History Test!");
    ...
    {  
        ...
        try
        {
            ...
            if(current==k)  
            {    
                String sql="select * from history order by RAND() Limit 4"; //selects all fields of table 'java_questions' 
    ...
    void showAnswerKey() //function to print answer key if requested for
    {
        try
        {
            ...
            for(int i=1;i<=10;i++)
            {
                String sq="select CorrectAns from history where ID="+i+"";
                ResultSet rs2 = stmt.executeQuery(sq);
                rs2.next();
