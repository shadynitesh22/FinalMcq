 ...
 * @author Mili Risal
 */
public class MovieExams extends JFrame implements ActionListener{
    ...
    public MovieExams(){
        ...
        lblwelcome=new JLabel("Welcome to Movie Trivia!");
    ...
    {  
        ...
        try
        {
            ...
            if(current==k)  
            {    
                String sql="select * from movie order by RAND() Limit 4"; //selects all fields of table 'java_questions' 
    ...
    void showAnswerKey() //function to print answer key if requested for
    {
        try
        {
            ...
            for(int i=1;i<=10;i++)
            {
                String sq="select CorrectAns from movie where ID="+i+"";
                ResultSet rs2 = stmt.executeQuery(sq);
                rs2.next();
